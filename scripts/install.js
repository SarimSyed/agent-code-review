...
function loadPackageJson() {
  ...
  if (!pkg.acrConfig || !pkg.acrConfig.urlPattern) {
    throw new Error("Missing acrConfig.urlPattern in package.json");
  }
  return pkg;
}

function resolveVersion(pkg) {
  const envVersion = process.env.ACR_VERSION;
  if (envVersion) {
    const v = envVersion.startsWith("v") ? envVersion.slice(1) : envVersion;
    info(`Using pinned version from ACR_VERSION: ${v}`);
    return v;
  }
...
async function main() {
  info("Agent Code Review Installer");
  info("=========================");
  ...
  if (!IS_WINDOWS) {
    const jsWrapper = path.join(binDir, "acr.js");
    if (fs.existsSync(jsWrapper)) {
      try {
        fs.chmodSync(jsWrapper, 0o755);
      } catch (e) {
        warn(`Could not make acr.js executable: ${e.message}`);
      }
    }
  }
  ...
  info("");
  info("Agent Code Review is ready!");
  info("");
  info("Quick start:");
  info("  acr version             Show version info");
  info("  acr doctor              Check local agent integration");
  info("  acr review              Prepare an agent-delegated review");
}
