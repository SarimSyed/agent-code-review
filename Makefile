...
define BUILD_PLATFORM
	GOOS=$(1) GOARCH=$(2) CGO_ENABLED=0 $(GO) build $(BUILD_FLAGS) -ldflags "$(RELEASE_LD_FLAGS)" \
...
# ── Development targets ──────────────────────────────────────────────────────
build:
	$(GO) build $(BUILD_FLAGS) -ldflags "$(LD_FLAGS)" -o $(DIST_DIR)/$(BINARY_NAME) ./cmd/acr

PACKAGES := $(shell GOFLAGS=-buildvcs=false $(GO) list ./... | grep -v /extensions/)
