...
type packageJSON struct {
	AcrConfig acrConfig `json:"acrConfig"`
}
...
func TestURLPatternNoVersionInFilename(t *testing.T) {
	...
	filename := extractFilenameFromURL(pkg.AcrConfig.URLPattern)
...
func TestReleaseYmlMatchesURLPattern(t *testing.T) {
	...
	// Match: BIN_NAME=acr-${{ matrix.goos }}-${{ matrix.goarch }}
	re := regexp.MustCompile(`BIN_NAME=(acr[^\n]*\}\})`)
...
func TestMakefileBuildMatchesURLPattern(t *testing.T) {
	...
	for _, p := range platforms {
		...
		rendered = strings.ReplaceAll(rendered, "$(BINARY_NAME)", "acr")
...
func TestChecksumFilenameNoVersion(t *testing.T) {
	pkg := loadPackageJSON(t)
	filename := extractFilenameFromURL(pkg.AcrConfig.ChecksumPattern)
